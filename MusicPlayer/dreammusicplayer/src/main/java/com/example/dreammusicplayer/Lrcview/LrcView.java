package com.example.dreammusicplayer.Lrcview;
...
public class LrcView extends View implements ILrcView {
	 ...
	 * 无歌词数据的时候 显示的默认文字
	 **/
	private static final String DEFAULT_TEXT = "梦想之音，传播好声音";
	/**
	 * 默认文字的字体大小
	 **/
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = minRaw; i <= maxRaw; i++) {
			if (i == mCurRow) {//画高亮歌词
				...
				float textSize = mCurSizeForOtherLrc + (mCurSizeForHightLightLrc -
				...
				if (i == mLastRow) {//画高亮歌词的上一句
					//因为有缩放效果，所有需要动态设置歌词的字体大小
					float textSize = mCurSizeForHightLightLrc - (mCurSizeForHightLightLrc -
						   mCurSizeForOtherLrc) * mCurFraction;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				if (!canDrag) {
					if (Math.abs(event.getRawY() - firstY) > mTouchSlop && Math.abs(event
						   .getRawY() - firstY) > Math.abs(event.getRawX() - lastX)) {
						canDrag = true;
				...
				if (canDrag) {
					...
					} else if (getScrollY() - offset > mLrcRows.size() * (mCurSizeForOtherLrc
					...
					lastY = event.getRawY();
					int currentRow = (int) (getScrollY() / (mCurSizeForOtherLrc +
						   mCurPadding));
			...
			case MotionEvent.ACTION_CANCEL:
				if (!canDrag) {
					...
					} else if (getScrollY() > mLrcRows.size() * (mCurSizeForOtherLrc +
						   mCurPadding) - mCurPadding) {
						smoothScrollTo((int) (mLrcRows.size() * (mCurSizeForOtherLrc +
							   mCurPadding) - mCurPadding), DURATION_FOR_ACTION_UP);
					}
	...
	public void setLrcRows(List<LrcRow> lrcRows) {
		if (lrcRows == null || lrcRows.size() <= 0) return;
		reset();
			...
			if (progress >= mLrcRows.get(i).getTime()) {
				if (mCurRow != i) {
					...
					if (fromSeekBarByUser) {
						...
						scrollTo(getScrollX(), (int) (mCurRow * (mCurSizeForOtherLrc +
							   mCurPadding)));
					} else {
					...
					float textWidth = mPaintForHighLightLrc.measureText(mLrcRows.get(mCurRow)
						   .getContent());
					...
					if (textWidth > getWidth()) {
						...
						startScrollLrc(getWidth() - textWidth, (long) (mLrcRows.get(mCurRow)
							   .getTotalTime() * 0.6));
					}
					invalidate();
				}
